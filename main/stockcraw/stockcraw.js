...
//数据采集和数据解析分开处理
function getStock() {
    request.get('http://hqres.eastmoney.com/EMQuote_Center3.0/js/gridlist.min.js?v=180925193855968',
        function (err, res, body) {
            let tokenReg = RegExp('token=[0-9,A-Z,a-z]*');
            let tokenRes = tokenReg.exec(String(body));
            let token = tokenRes[0].replace('token=', '');
            // console.log(token);
            request.get('http://nufm.dfcfw.com/EM_Finance2014NumericApplication/JS.aspx?type=CT&sty=FCRH&token=' + token + '&cmd=0000011,3990012,0003001,DJIA_UI,SPX_UI,NDX_UI,2CN225_UI,2C',
                function (err, res, body) {
